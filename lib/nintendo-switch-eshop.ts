...
export const getGamesAmerica = async (
  options: USRequestOptions = { shop: "ncom", limit: US_GAME_LIST_LIMIT },
  offset: number = 0,
  games: GameUS[] = []
): Promise<GameUS[]> => {
  ...
  const addedOptions: { limit: number; offset: number; shop?: string } = {
    limit: options.limit,
    offset
  };

  if (options.shop !== "all") {
    addedOptions.shop = options.shop;
  }

  try {
    const gamesUS = await fetch(
      `${US_GET_GAMES_URL}?${stringify({
        ...addedOptions,
        ...US_GET_GAMES_OPTIONS
      })}`
    );
    ...
    const accumulatedGames: GameUS[] = arrayRemoveDuplicates(
      games.concat(filteredResponse.games.game),
      "slug"
    );
...
export const getGamesJapan = async (): Promise<GameJP[]> => {
  try {
    ...
    if (!currentGamesJP.ok)
      throw new Error("JP_current_games_US_games_request_failed");
    if (!comingGamesJP.ok)
      throw new Error("JP_coming_games_US_games_request_failed");
    ...
    if (/(?:JP_current_games_US_games_request_failed)/i.test(err.toString()))
      throw new EshopError("Fetching of Current JP Games failed");
    if (/(?:JP_coming_games_US_games_request_failed)/i.test(err.toString()))
      throw new EshopError("Fetching of Upcoming JP Games failed");
    return err;
  }
};
...
export const getGamesEurope = async (
  options: EURequestOptions = {
    limit: EU_GAME_LIST_LIMIT,
    locale: EU_DEFAULT_LOCALE
  }
): Promise<GameEU[]> => {
...
export const getShopsByCountryCodes = async (
  ...
  try {
    const countryList: Country[] = countryCodes.map(
      (code: string) =>
        countries.all.filter((country: Country) => country.alpha2 === code)[0]
    );
